...
impl HoverProvider {
    ...
    fn get_keyword_info(&self, word: &str) -> Option<String> {
        match word {
            ...
            "can" => Some("**can** Keyword\n\nDeclares a capability (nominal method contract) or claims one on a class.\n\n**Declare a capability:**\n```clean\ncan Draw:\n\tdraw()\n\nstring describe()\n```\n\n**Claim capabilities on a class** (after the optional `is Parent`):\n```clean\nclass Circle is Shape can Draw, Describable\n\tnumber radius\n\n\tfunctions:\n\t\tpublic:\n\t\t\tdraw()\n\t\t\t\tprint(\"circle\")\n```\n\nCapabilities are **nominal** — a class must explicitly claim the capability. Missing methods → SEM011. Undefined capability → SEM012. Signature mismatch → SEM013.\n\n**v1 note:** calling a method on a capability-typed value (e.g. `Draw thing; thing.draw()`) is a compile error today. Runtime dispatch ships in a follow-up.\n\n---\n⚡ *Built-in*".to_string()),
