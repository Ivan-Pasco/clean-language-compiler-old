...
impl DefaultParameterManager {
    ...
    fn register_parameter_default_functions(&self, codegen: &mut CodeGenerator) -> Result<(), CompilerError> {
    ...
    fn register_input_block_functions(&self, codegen: &mut CodeGenerator) -> Result<(), CompilerError> {
    ...
    fn register_default_evaluation_functions(&self, codegen: &mut CodeGenerator) -> Result<(), CompilerError> {
    ...
    fn register_parameter_utility_functions(&self, codegen: &mut CodeGenerator) -> Result<(), CompilerError> {
    ...
    fn generate_get_field_with_default(&self) -> Vec<Instruction> {
        vec![
            ...
            Instruction::Else,
                ...
                Instruction::LocalGet(3), // field_ptr
                Instruction::I32Load(MemArg { offset: 8, align: 2, memory_index: 0 }), // Load value pointer
        ...
        vec![
            ...
            Instruction::Else,
                ...
                Instruction::If(BlockType::Result(ValType::I32)),
                    ...
                    Instruction::LocalGet(0),
                    Instruction::I32Load(MemArg { offset: 4, align: 2, memory_index: 0 }), // Function index
    ...
    fn generate_evaluate_number_default(&self) -> Vec<Instruction> {
        vec![
            ...
            Instruction::If(BlockType::Result(ValType::F64)),
                // Load constant value
                Instruction::LocalGet(0),
                Instruction::F64Load(MemArg { offset: 4, align: 3, memory_index: 0 }),
            Instruction::Else,
                Instruction::LocalGet(1),
                Instruction::I32Const(2), // Function call expression
                Instruction::I32Eq,
    ...
    fn generate_was_provided(&self) -> Vec<Instruction> {
        vec![
            ...
            Instruction::LocalGet(0),
            Instruction::I32Load(MemArg { offset: 12, align: 2, memory_index: 0 }), // Provided flag at offset 12
        ]
    }
    ...
    fn generate_set_function_defaults(&self) -> Vec<Instruction> {
        vec![
            ...
            Instruction::LocalGet(0),
            Instruction::I32Load(MemArg { offset: 16, align: 2, memory_index: 0 }), // Param count at offset 16
    ...
    fn generate_apply_call_defaults(&self) -> Vec<Instruction> {
        vec![
            ...
            // Get parameter count from function
            Instruction::LocalGet(0),
            Instruction::I32Load(MemArg { offset: 16, align: 2, memory_index: 0 }),
    ...
    fn get_string_length_function_index(&self) -> u32 { 1100 }
    fn get_list_copy_function_index(&self) -> u32 { 1101 }
    fn get_allocate_function_index(&self) -> u32 { 1102 }
    fn get_initialize_input_defaults_function_index(&self) -> u32 { 1103 }
    fn get_apply_defaults_loop_function_index(&self) -> u32 { 1104 }
    fn get_find_input_field_function_index(&self) -> u32 { 1105 }
    fn get_count_valid_fields_function_index(&self) -> u32 { 1106 }
    fn get_empty_string_function_index(&self) -> u32 { 1107 }
    fn get_apply_function_defaults_function_index(&self) -> u32 { 1108 }
    fn get_merge_args_with_defaults_function_index(&self) -> u32 { 1109 }
    fn get_count_required_function_index(&self) -> u32 { 1110 }
}
...
mod tests {
    ...
    fn test_parameter_default_generation() {
        ...
        assert!(int_default.iter().any(|inst| matches!(inst, Instruction::If(_))));
        
        let num_default = default_manager.generate_number_param_default();
        assert!(!num_default.is_empty());
        // Should use NaN check for numbers
        assert!(num_default.iter().any(|inst| matches!(inst, Instruction::F64Ne)));
        ...
        let if_count = str_default.iter().filter(|inst| matches!(inst, Instruction::If(_))).count();
        assert!(if_count >= 2);
        ...
        assert!(bool_default.iter().any(|inst| matches!(inst, Instruction::I32Const(-1))));
    }
    ...
    fn test_input_block_generation() {
        ...
        assert!(create_input.iter().any(|inst| matches!(inst, Instruction::Call(_))));
        ...
        let if_count = get_field.iter().filter(|inst| matches!(inst, Instruction::If(_))).count();
        assert!(if_count >= 2);
    ...
    fn test_default_evaluation_generation() {
        ...
        assert!(eval_int.iter().any(|inst| matches!(inst, Instruction::CallIndirect { .. })));
        
        let eval_num = default_manager.generate_evaluate_number_default();
        assert!(!eval_num.is_empty());
        // Should handle F64 values
        assert!(eval_num.iter().any(|inst| matches!(inst, Instruction::F64Load(_))));
        assert!(eval_num.iter().any(|inst| matches!(inst, Instruction::F64Const(_))));
        ...
        let call_count = eval_str.iter().filter(|inst| matches!(inst, Instruction::Call(_))).count();
        assert!(call_count >= 1);
    ...
    fn test_parameter_utility_generation() {
        ...
        let store_count = create_param.iter().filter(|inst| matches!(inst, Instruction::I32Store(_))).count();
        ...
        let load_count = apply_call.iter().filter(|inst| matches!(inst, Instruction::I32Load(_))).count();
        ...
        assert!(matches!(count_required[count_required.len() - 1], Instruction::Call(_)));
    }
}