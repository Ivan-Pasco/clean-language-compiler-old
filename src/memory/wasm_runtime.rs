...
pub fn init_memory_manager(layout: MemoryLayout) -> MemoryResult<()> {
    let mut manager = GLOBAL_MEMORY_MANAGER.lock()
        .map_err(|_| CompilerError::memory_error("Failed to acquire memory manager lock", None, None))?;
    ...
    F: FnOnce(&mut MemoryManager) -> MemoryResult<R>,
{
    ...
    let manager_ref = manager.as_mut()
...
pub fn mem_alloc(type_id: i32, size: i32) -> i32 {
    match with_memory_manager(|manager| {
        manager.allocate(type_id as TypeId, size as u32)
    }) {
...
mod tests {
    ...
    fn test_memory_stats() {
        ...
        assert!(stats.total_allocated >= 0, "Total allocated should be non-negative");
    }
}