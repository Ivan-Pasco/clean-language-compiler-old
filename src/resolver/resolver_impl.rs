...
impl NameResolver {
    ...
    fn register_builtin_functions(&mut self) {
        ...
        // Endpoint test bridge functions — generated by HIR builder for `test "name" METHOD "path"` blocks.
        // Registered here so the resolver accepts the synthetic calls emitted by build_endpoint_test_body().
        self.register_builtin_fn(
            "_test_http_request_clean",
            vec![
                HirType::String, // method
                HirType::String, // path
                HirType::String, // body
                HirType::String, // header key
                HirType::String, // header value
            ],
            Some(HirType::Integer), // returns opaque handle
            builtin_location.clone(),
        );
        self.register_builtin_fn(
            "_test_response_status",
            vec![HirType::Integer], // handle
            Some(HirType::Integer),
            builtin_location.clone(),
        );
        self.register_builtin_fn(
