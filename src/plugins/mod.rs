...
pub struct PluginExpansion {
    ...
    /// Class definitions generated by the plugin (e.g., ORM models)
    pub classes: Vec<crate::ast::Class>,
    /// Top-level `can` capability declarations emitted by the plugin via the
    /// `_emit_capability` bridge (typed-emission §3.18 / Amendment 13).
    /// Merged into `program.capabilities` by the expander with name-based
    /// dedup: identical declarations across plugins/files are silently
    /// coalesced; signature conflicts produce PLUGIN002.
    pub capabilities: Vec<crate::ast::Capability>,
