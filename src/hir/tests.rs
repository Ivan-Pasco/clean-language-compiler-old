...
mod hir_builder_tests {
    ...
    fn test_simple_function_to_hir() {
        ...
        assert!(result.is_ok(), "Simple function should convert to HIR successfully");
        ...
        assert!(matches!(hir.functions[0].return_type, Some(HirType::Integer)));
    }
    ...
    fn test_start_function_to_hir() {
        ...
        let result = HirBuilder::build(ast);
        assert!(result.is_ok(), "Start function should convert to HIR successfully");
                        ...
                        body: BlockNode {
                            statements: vec![StatementNode::Assignment {
                                target: ExpressionNode::FieldAccess {
                                    object: Box::new(ExpressionNode::This { location: test_location() }),
    ...
    fn test_expression_desugaring() {
        ...
        let result = HirBuilder::build(ast);
        assert!(result.is_ok(), "Complex expressions should desugar correctly");

        let hir = result.unwrap().hir;
        let start_fn = hir.start_function.unwrap();
        assert_eq!(start_fn.body.statements.len(), 1);
        
        if let HirStatement::VariableDeclaration { initializer: Some(expr), .. } = &start_fn.body.statements[0] {
    ...
    fn test_method_call_desugaring() {
        ...
        if let HirStatement::VariableDeclaration { initializer: Some(expr), .. } = &start_fn.body.statements[1] {
    ...
    fn test_type_inference_generation() {
                ...
                body: BlockNode {
                    statements: vec![
                        StatementNode::VariableDeclaration {
                            ...
                            var_type: TypeNode::Inferred { location: test_location() },
                            initializer: Some(Box::new(ExpressionNode::Literal {
                                value: Value::Integer(42),
                                location: test_location(),
                            })),
        ...
        assert!(validation_result.type_inference_count > 0, "Should generate type inference variables");
...
mod hir_validation_tests {
    ...
    fn test_undefined_variable_error() {
        ...
        let result = HirValidator::validate(&hir);
        assert!(result.is_err(), "Undefined variable should cause validation error");
        
        let errors = result.unwrap_err();
        assert!(errors.iter().any(|e| e.message().contains("Undefined variable 'undefined'")));
    }
    ...
    fn test_duplicate_function_validation() {
        ...
        let result = HirValidator::validate(&hir);
        assert!(result.is_err(), "Duplicate functions should cause validation error");
    ...
    fn test_circular_inheritance_error() {
        ...
        let result = HirValidator::validate(&hir);
        assert!(result.is_err(), "Circular inheritance should cause validation error");
    ...
    fn test_this_reference_validation() {
                                ...
                                statements: vec![
                                    HirStatement::Return {
                                        value: Some(HirExpression::FieldAccess {
                                            object: Box::new(HirExpression::This { location: test_location() }),
        ...
        let result = HirValidator::validate(&hir);
        assert!(result.is_ok(), "Valid 'this' reference should pass validation");
    }
    ...
    fn test_this_outside_class_error() {
            ...
            functions: vec![
                HirFunction {
                    ...
                    body: HirBlock {
                        statements: vec![
                            HirStatement::Return {
                                value: Some(HirExpression::This { location: test_location() }),
                                location: test_location(),
                            }
                        ],
                        location: test_location(),
        ...
        let result = HirValidator::validate(&hir);
        assert!(result.is_err(), "'this' outside class should cause validation error");
        
        let errors = result.unwrap_err();
        assert!(errors.iter().any(|e| e.message().contains("'this' can only be used inside a class")));
    }
}
...
mod integration_tests {
    ...
    fn test_end_to_end_simple_program() {
        ...
        assert!(hir_result.is_ok(), "Simple program should build HIR successfully");
        ...
        assert!(validation.is_ok(), "Simple program HIR should validate successfully");
    ...
    fn test_end_to_end_class_with_inheritance() {
        ...
        // Build HIR
        let hir_result = HirBuilder::build(ast);
        assert!(hir_result.is_ok(), "Class inheritance should build HIR successfully");

        let hir = hir_result.unwrap().hir;

        // Validate HIR
        let validation = HirValidator::validate(&hir);
        assert!(validation.is_ok(), "Class inheritance HIR should validate successfully");
