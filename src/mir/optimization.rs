...
impl MirOptimizer {
    ...
    pub fn optimize(&mut self, mut program: MirProgram) -> Result<(MirProgram, OptimizationStats), Vec<CompilerError>> {
...
impl ConstantFoldingPass {
    ...
    fn try_fold_binary_op(&self, op: &MirBinaryOp, left: &MirConstant, right: &MirConstant) -> Option<MirConstant> {
        match (left, right) {
            (MirConstant::Integer(a), MirConstant::Integer(b)) => {
                match op {
            ...
            (MirConstant::Boolean(a), MirConstant::Boolean(b)) => {
                match op {
        ...
        for block in function.blocks.values_mut() {
            ...
            for (i, instruction) in block.instructions.iter_mut().enumerate() {
                match &mut instruction.operation {
                    MirOperation::Copy { source: MirOperand::Constant(constant) } => {
...
impl DeadCodeEliminationPass {
    ...
    fn mark_operands_live(&mut self, operation: &MirOperation, function: &MirFunction) {
        match operation {
            ...
            MirOperation::Call { function: func, arguments } => {
    ...
    fn optimize_function(&mut self, function: &mut MirFunction) -> Result<PassStats, CompilerError> {
        ...
        for block in function.blocks.values() {
            ...
            match &block.terminator {
                MirTerminator::Return { value: Some(operand) } => {
                    self.mark_operand_live(operand, function);
                }
