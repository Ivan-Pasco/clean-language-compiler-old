...
fn test_final_lir_to_wasm_complete() {
    ...
    match wasm_result {
        Ok(wasm_bytes) => {
            assert!(
                wasm_bytes.len() > 30,
                "Generated WASM should have reasonable size"
            );
            println!(
                "✅ Complete LIR to WASM generation: {} bytes",
                wasm_bytes.len()
            );
        }
        Err(e) => {
...
fn test_all_optimization_functions() {
    ...
    assert!(
        ...
        "Dead code elimination should work"
    );
    ...
    assert!(
        ...
        "Control flow optimization should work"
    );
...
fn test_all_validation_functions() {
    ...
    assert!(
        validate_hir(&hir_program).is_ok(),
        "HIR validation should work"
    );
    ...
    assert!(
        validate_mir(&mir_program).is_ok(),
        "MIR validation should work"
    );
    ...
    assert!(
        validate_lir(&lir_program).is_ok(),
        "LIR validation should work"
    );
...
fn test_mir_instruction_varieties() {
    ...
    let instructions = vec![
        ...
        MIRInstruction::Store(
            MIROperand::Global("result".to_string()),
            MIROperand::Local(3),
        ),
