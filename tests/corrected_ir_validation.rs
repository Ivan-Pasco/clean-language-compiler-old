...
fn test_correct_mir_structure() {
    ...
    basic_blocks.insert(0, MIRBasicBlock {
        id: 0,
        instructions: vec![],
...
fn test_lir_comprehensive_validation() {
    ...
    match wasm_result {
        Ok(wasm_bytes) => {
            assert!(wasm_bytes.len() > 20, "Generated WASM should have substantial size");
            println!("✅ Comprehensive LIR to WASM: {} bytes generated", wasm_bytes.len());
        }
        Err(e) => {
...
fn test_complete_ir_pipeline_functionality() {
    ...
    let mut mir_functions = HashMap::new();
    mir_functions.insert("add_test".to_string(), MIRFunction {
