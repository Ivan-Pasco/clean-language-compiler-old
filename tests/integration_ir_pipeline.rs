...
fn test_basic_program_ir_pipeline() {
    ...
    let wasm_bytes = wasm_gen
        .generate_wasm_module(lir)
        .expect("Failed to generate WebAssembly");

    assert!(
        !wasm_bytes.is_empty(),
        "WebAssembly output should not be empty"
    );
}
...
fn test_memory_management_in_lir() {
    ...
    let has_memory_ops = main_func.instructions.iter().any(|inst| {
        matches!(
            inst,
            LIRInstruction::MemAlloc | LIRInstruction::MemRetain | LIRInstruction::MemRelease
        )
    });
