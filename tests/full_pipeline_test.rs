...
fn test_manual_ast_to_full_pipeline() {
    ...
    let ast_program = Program {
        ...
        declarations: vec![Declaration::Function(Function {
            ...
            body: vec![Statement::Return(Some(Expression::IntegerLiteral(42)))],
            modifier: vec![],
    ...
    match hir_result {
        Ok(hir_program) => {
            ...
            assert!(
                !hir_program.functions.is_empty(),
                "HIR should contain functions"
            );
            ...
            match mir_result {
                Ok(mir_program) => {
                    ...
                    assert!(
                        !mir_program.functions.is_empty(),
                        "MIR should contain functions"
                    );
                    ...
                    match lir_result {
                        Ok(lir_program) => {
                            ...
                            assert!(
                                !lir_program.functions.is_empty(),
                                "LIR should contain functions"
                            );

                            // Validate LIR structure
                            let main_function = lir_program
                                .functions
                                .iter()
                                .find(|f| f.name == "main")
                            ...
                            assert!(
                                !main_function.instructions.is_empty(),
                                "LIR function should have instructions"
                            );

                            // Should have return type
                            assert!(
                                main_function.return_type.is_some(),
                                "Main function should have return type"
                            );
...
fn test_ir_optimization_pipeline() {
    ...
    println!(
        "Original MIR function instructions: {} blocks",
        mir_program.functions[0].basic_blocks.len()
    );
...
fn test_memory_layout_integration() {
    ...
    assert!(
        layout.heap_start > layout.stack_start,
        "Heap should start after stack"
    );
    assert!(
        layout.initial_pages > 0,
        "Should have at least one initial page"
    );
    assert!(
        layout.max_pages.unwrap_or(1) >= layout.initial_pages,
        "Max pages should be >= initial"
    );
