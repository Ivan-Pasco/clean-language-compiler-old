...
fn compile_to_wasm(input_file: &str, output_file: &str) -> Result<CompilationResult, String> {
    let output = Command::new("cargo")
        .args(&[
            "run", "--bin", "clean-language-compiler", 
            "compile", "-i", input_file, "-o", output_file
        ])
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .output()
...
pub fn assert_execution(input_file: &str, expected_output: &str, expected_exit_code: i32) {
    match execute_clean_file(input_file) {
        Ok(result) => {
            ...
            assert_eq!(
                result.stdout.trim(), expected_output.trim(),
                "Output mismatch for {}: expected '{}', got '{}'\nStderr: {}",
                input_file, expected_output, result.stdout, result.stderr
            );
            
            println!("✅ {} executed successfully in {:?}", input_file, result.duration);
        }
        Err(e) => {
...
pub mod test_utils {
    ...
    pub fn run_comprehensive_execution_tests() {
        ...
        println!("🚀 Running comprehensive execution tests on {} files:", test_files.len());
        ...
        for file in test_files {
            ...
            match execute_clean_file(&file) {
                Ok(result) => {
                    println!("✅ Success (exit: {}, time: {:?})", result.exit_code, result.duration);
                    successful += 1;
                }
                Err(error) => {
